...
import { Eye, BarChart3, MessageSquare, ThumbsUp } from 'lucide-react'
...
const TotalProperty = () => {
    ...
    const getOwnBlog = async () => {
        try {
            const res = await axios.get(`https://blogging-application-ox2h.onrender.com/api/v1/blog/get-own-blogs`, { withCredentials: true })
            if (res.data.success) {
    ...
    const getTotalLikes = async () => {
        try {
            const res = await axios.get(`https://blogging-application-ox2h.onrender.com/api/v1/blog/my-blogs/likes`, { withCredentials: true })
            if (res.data.success) {
            ...
            <div className='flex flex-col md:flex-row justify-around gap-3 md:gap-7 '>
                {
                    stats.map((stat) => {
                        return <Card key={stat.title} className="w-full dark:bg-gray-800 -space-y-5">
                            <CardHeader className="flex flex-row items-center justify-betwee
                            n space-y-0 pb-2">
